...
#define T_AREA_HEIGHT	80  /* Height of text window */
...
//static void x11_build_searchbar(void); 
...
static void menutext_(XftDraw* draw, int xc, int yc, const char *text);
...
static void map_button(int bnum) {
    ...
    if (button_state.button[bnum].type != BUTTON_SEPARATOR) {
        ...
        button_state.button[bnum].win = XCreateSimpleWindow(
            ...
            x11_convert_to_xcolor(t_color::predef_colors[BLACK]) /* COLOR NOTE: each time pressing POI button,
                the buttons after will disappear for a second, this is the color behind those buttons 
                (better change to the same as the UI color) */
            );
...
draw_message(void) {
    ...
    if (gl_state.disp_type == SCREEN) {
        ...
        XSetForeground(x11_state.display, x11_state.gc_menus, 
                    x11_convert_to_xcolor(t_color::predef_colors[BLACK]));//COLOR NOTE: left/top of the message box
        XDrawRectangle(x11_state.display, x11_state.textarea, x11_state.gc_menus, 0, 0,
            trans_coord.top_width - MWIDTH, T_AREA_HEIGHT);
        XSetForeground(x11_state.display, x11_state.gc_menus, 
                    x11_convert_to_xcolor(t_color::predef_colors[BLACK]));//COLOR NOTE: right/bot of the message box
...
//    XftDrawDestroy(x11_state.searchbar_draw);
...
build_default_menu(void) {
    int i, xcen, x1, y1, bwid = 0, bheight, space;
    const int NUM_ARROW_BUTTONS = 0, NUM_STANDARD_BUTTONS = 4, SEPARATOR_BUTTON_INDEX = NUM_STANDARD_BUTTONS - 2;
    ...
    x11_state.menu = XCreateSimpleWindow(
        ...
        x11_convert_to_xcolor(t_color::predef_colors[BLACK]) //COLOR NOTE: right side UI
        );
...
//    menu_attributes.event_mask = 0;
    ...
    /* Rectangular buttons */ //NORMAL BUTTON
    y1 = 30;
    xcen = 51;
    x1 = xcen - bwid / 2;
    space = 15;
    ...
    strcpy(button_state.button[2].text, "---1");
    strcpy(button_state.button[3].text, "Exit");


    button_state.button[0].fcn = zoom_in;
    button_state.button[1].fcn = zoom_out;
    button_state.button[2].fcn = NULL;
    button_state.button[3].fcn = proceed;
...
/* Helper function called by init_graphics(). Not visible to client program. */
static void x11_init_graphics(const char *window_name) {
    XInitThreads(); //multithreading support
                ...
                if (report.xbutton.window == x11_state.toplevel) {
                    ...
                    switch (report.xbutton.button) {
                        ...
                        case Button4: /* Scroll wheel rotated forward; screen does zoom_in */
                            ...
                            else if (act_on_mousebutton != NULL) {// If callback was set
                                act_on_mousebutton(x, y, button_info);
                            }
                            break;
...
static void x11_build_textarea(void) {
    ...
    x11_state.textarea = XCreateSimpleWindow(x11_state.display, x11_state.toplevel, 0,
        ...
        x11_convert_to_xcolor(t_color::predef_colors[WHITE])); //COLOR NOTE: message UI (under the map)
...
static void menutext_(XftDraw* draw, int xc, int yc, const char *text) {
    ...
    font_ptr menu_font = gl_state.font_info.get_font_info(MENU_FONT_SIZE + 8, 0);
...
static void x11_drawbut(int bnum) {
    ...
    if (ispressed) {
        XSetForeground(x11_state.display, x11_state.gc_menus, 
                x11_convert_to_xcolor(t_color::predef_colors[WHITE]));//COLOR NOTE: top/left of pressed button
    } else {
        XSetForeground(x11_state.display, x11_state.gc_menus, 
                x11_convert_to_xcolor(t_color::predef_colors[LIGHTGREY]));//COLOR NOTE: top/left of unpressed button
    }
    ...
    if (ispressed) {
        XSetForeground(x11_state.display, x11_state.gc_menus, 
                x11_convert_to_xcolor(t_color::predef_colors[WHITE])); //COLOR NOTE: bot/right of pressed button
    } else {
        XSetForeground(x11_state.display, x11_state.gc_menus, 
                x11_convert_to_xcolor(t_color::predef_colors[LIGHTGREY])); //COLOR NOTE: bot/right of unpressed button
    }
    mypoly[0].x = 0;
    mypoly[0].y = height;
    mypoly[1].x = width;
    ...
    if (ispressed) {
        XSetForeground(x11_state.display, x11_state.gc_menus,
            x11_convert_to_xcolor(t_color::predef_colors[BLACK]) // COLOR NOTE: pressed button
            );
    } else {
        XSetForeground(x11_state.display, x11_state.gc_menus,
            x11_convert_to_xcolor(t_color::predef_colors[WHITE]) // COLOR NOTE: unpressed buttons
            );
    }
    ...
    if (button_state.button[bnum].type == BUTTON_POLY) {
        ...
        XSetForeground(x11_state.display, x11_state.gc_menus,
            x11_convert_to_xcolor(t_color::predef_colors[BLACK]) // COLOR NOTE: text of arrow buttons
            );
    ...
    if (button_state.button[bnum].type == BUTTON_TEXT) {
        ...
        menutext(button_state.button[bnum].draw, button_state.button[bnum].width / 2,
            45 / 2, button_state.button[bnum].text);
    }
}
...
static void x11_drawmenu(void) {
    ...
    XSetForeground(x11_state.display, x11_state.gc_menus, 
            x11_convert_to_xcolor(t_color::predef_colors[BLACK]) ); //COLOR NOTE: left/top of the menu
    XDrawRectangle(x11_state.display, x11_state.menu, x11_state.gc_menus, 0, 0, MWIDTH,
        trans_coord.top_height);
    XSetForeground(x11_state.display, x11_state.gc_menus, 
            x11_convert_to_xcolor(t_color::predef_colors[BLACK]) ); //COLOR NOTE: right/bot of the menu
...
void center_and_zoom(float x, float y, void (*draw_screen) (void),  double street_threshold) {
    
    double zoom_in_value = 1.1;
    double zoom_out_value = 1.1;
    double threshold = street_threshold;
    double og_xcen = ((trans_coord.xright + trans_coord.xleft) / 2);
    double og_ycen = ((trans_coord.ybot + trans_coord.ytop) / 2);
    
    if (x >= trans_coord.xleft && x <= trans_coord.xright && y >= trans_coord.ybot && y <= trans_coord.ytop && LOD_area_test(threshold)) {
        float x_change_world, y_change_world;

        x_change_world = x - og_xcen;
        y_change_world = y - og_ycen;
        ...
        pan_state.previous_x = xworld_to_scrn(x);
        pan_state.previous_y = yworld_to_scrn(y);
        
        return;
    }
    
    if (!LOD_area_test(0.1)) {
        while (!LOD_area_test(threshold)) {
            trans_coord.xleft = x - (x - trans_coord.xleft) / zoom_in_value;
            trans_coord.xright = x + (trans_coord.xright - x) / zoom_in_value;
            trans_coord.ytop = y - (y - trans_coord.ytop) / zoom_in_value;
            trans_coord.ybot = y + (trans_coord.ybot - y) / zoom_in_value;
        ...
        double distance = sqrt((x-og_xcen)*(x-og_xcen) + (y-og_ycen)*(y-og_ycen));
        
        double zoom_out_threshold = std::max(distance/10, 0.0025);
        if (distance < 0.01) {
            zoom_out_threshold = 0.0005;
        } else if (distance < 0.02) {
            zoom_out_threshold = 0.001;
        } else if (distance < 0.03) {
            zoom_out_threshold = 0.0015;
        }
        
        while (LOD_area_test(zoom_out_threshold)) {
        
            double zoom_out_xcen;
            double zoom_out_ycen;
            
            if (x < trans_coord.xleft) {
                zoom_out_xcen = trans_coord.xright;
            } else if (x > trans_coord.xright) {
                zoom_out_xcen = trans_coord.xleft;
            } else {
                zoom_out_xcen = og_xcen;
            }
            if (y < trans_coord.ybot) {
                zoom_out_ycen = trans_coord.ytop;
            } else if (x > trans_coord.ytop) {
                zoom_out_ycen = trans_coord.ybot;
            } else {
                zoom_out_ycen = og_ycen;
            }

            trans_coord.xleft = zoom_out_xcen - (zoom_out_xcen - trans_coord.xleft)* zoom_out_value;
            trans_coord.xright = zoom_out_xcen + (trans_coord.xright - zoom_out_xcen)* zoom_out_value;
            trans_coord.ytop = zoom_out_ycen - (zoom_out_ycen - trans_coord.ytop)* zoom_out_value;
            trans_coord.ybot = zoom_out_ycen + (trans_coord.ybot - zoom_out_ycen)* zoom_out_value;
        ...
        while (!LOD_area_test(threshold)) {
            ...
            double cur_xcen = ((trans_coord.xright + trans_coord.xleft) / 2);
            double cur_ycen = ((trans_coord.ybot + trans_coord.ytop) / 2);
            
            double zoom_in_xcen = x;
            double zoom_in_ycen = y;
            
            double delta_x = og_xcen - x;
            double delta_y = og_ycen - y;
            
            double over = 3;
            
            double bound_tolerance = 0.45;
            
            double xleft = trans_coord.xleft + (abs(cur_xcen - trans_coord.xleft)) * bound_tolerance;
            double xright = trans_coord.xright - (abs(cur_xcen - trans_coord.xright)) * bound_tolerance;
            double ybot = trans_coord.ybot + (abs(cur_ycen - trans_coord.ybot)) * bound_tolerance;
            double ytop = trans_coord.ytop - (abs(cur_ycen - trans_coord.ytop)) * bound_tolerance;
            
            if (!(x > xleft && x < xright)) {
                zoom_in_xcen = og_xcen - (over * delta_x);
            }
            
            if (!(y > ybot && y < ytop)) {
                zoom_in_ycen = og_ycen - (over * delta_y);
            }
            
            trans_coord.xleft = zoom_in_xcen - (zoom_in_xcen - trans_coord.xleft) / zoom_in_value;
            trans_coord.xright = zoom_in_xcen + (trans_coord.xright - zoom_in_xcen) / zoom_in_value;
            trans_coord.ytop = zoom_in_ycen - (zoom_in_ycen - trans_coord.ytop) / zoom_in_value;
            trans_coord.ybot = zoom_in_ycen + (trans_coord.ybot - zoom_in_ycen) / zoom_in_value;
